...
#pragma MESSAGE DISABLE C1106 /*desactive les messages d'erreur */
// R4R3R2R1 R0G5G4G3 G2G1G0B4 B3B2B1B0
typedef union {
	 word Word; //16bits
	 struct {
    union {
      byte Byte;
      	  struct {
    	  	  	  byte GREEN3     :1;      /* GREEN Bit 3 */
    	  	  	  byte GREEN4     :1;      /* GREEN Bit 4 */
    	  	  	  byte GREEN5     :1;      /* GREEN Bit 5 */
    	  	  	  byte RED0       :1;      /* RED Bit 0 */
    	  	  	  byte RED1       :1;      /* RED Bit 1 */
    	  	  	  byte RED2       :1;      /* RED Bit 2 */
    	  	  	  byte RED3       :1;      /* RED Bit 3 */
    	  	  	  byte RED4       :1;      /* RED Bit 4 */
      	  	  	  }Bits;
      	  } MSB_COLOR; 
...
#define GREEN3 _COLOR16BITS.Overlap.MSB_COLOR.Bits.GREEN3
   	   union {
   	     byte Byte; //8bits
   	     struct {
   	    		byte BLUE0      :1;         /* BLUE Bit 0 */
   	    		byte BLUE1      :1;         /* BLUE Bit 1 */
   	    		byte BLUE2      :1;         /* BLUE Bit 2 */
   	    		byte BLUE3      :1;         /* BLUE Bit 3 */
   	    		byte BLUE4      :1;         /* BLUE Bit 4 */
   	    		byte GREEN0     :1;         /* GREEN Bit 0 */
   	    		byte GREEN1     :1;         /* GREEN Bit 1 */
   	    		byte GREEN2     :1;         /* GREEN Bit 2 */		
   	      } Bits;
   	    } LSB_COLOR;
